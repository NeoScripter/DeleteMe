...
export function useCarousel<T>({ slides, offset, animationDuration = 500, breakpoints }: UseCarouselOptions<T>): UseCarouselReturn<T> {
    ...
    const getOffset = () => {
        ...
        const config = breakpoints.find((bp) => width < bp.screen) ?? breakpoints.at(-1)!;
    ...
    const handleSlide = (direction: 1 | -1) => {
        ...
        setTimeout(() => {
            ...
            setCarouselSlides((prev) => (direction === 1 ? [...prev.slice(1), prev[0]] : [prev[prev.length - 1], ...prev.slice(0, -1)]));
